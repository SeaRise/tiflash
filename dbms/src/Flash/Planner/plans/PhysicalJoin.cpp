...
void PhysicalJoin::transformImpl(DAGPipeline & pipeline, Context & context, size_t max_streams)
{
    ...
    dag_analyzer.appendJoin(chain, build_query, columns_added_by_join);
    ...
    for (auto & c : schema)
    {
        ...
        /// it is guaranteed by its children physical plan
        project_cols.emplace_back(c.name, c.name);
    }
    ExpressionActionsPtr project = generateProjectExpressionActions(probe_pipeline.firstStream(), context, project_cols);
    executeExpression(probe_pipeline, project, log, "remove useless column after join");
}
