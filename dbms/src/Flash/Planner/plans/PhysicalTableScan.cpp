...
{
    auto schema = genNamesAndTypes(table_scan);
...
void PhysicalTableScan::transformImpl(DAGPipeline & pipeline, Context & context, size_t max_streams)
{
    ...
    assert(storage_schema.size() == schema.size());
    NamesWithAliases project_cols;
    for (size_t i = 0; i < schema.size(); ++i)
    {
        assert(schema[i].type->equals(*storage_schema[i].type));
        project_cols.emplace_back(storage_schema[i].name, schema[i].name);
    }
    ExpressionActionsPtr schema_project = generateProjectExpressionActions(pipeline.firstStream(), context, project_cols);
